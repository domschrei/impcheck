...
The clause fingerprinting approach relies on a confidential 128-bit key $K$ shared among all trusted parties. In the current version of ImpCheck, $K$ is generated by each trusted executable at the beginning of its execution. The key generation is based on (a) some "salt" decided at compilation time and (b) an optional 64-bit unsigned number $s$ provided as a command-line argument (`-key-seed=`$s$). Note that all parser, checker, and confirmer processes involved in a solving attempt must operate on the same $K$.
